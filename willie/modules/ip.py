...
def configure(config):
    ...
    if config.option('Configure a custom location for the GeoIP db?', False):
        ...
        config.interactive_add(
            'ip',
            'GeoIP_db_path',
            'Full path to the GeoIP database',
            None
        )
...
def _find_geoip_db(bot):
    ...
    if config.has_section('ip') and config.ip.GeoIP_db_path is not None:
        if os.path.isfile(
            os.path.join(config.ip.GeoIP_db_path, 'GeoLiteCity.dat')
        ) and os.path.isfile(
            os.path.join(config.ip.GeoIP_db_path, 'GeoIPASNum.dat')
        ):
    ...
    if os.path.isfile(
        os.path.join(bot.config.homedir, 'GeoLiteCity.dat')
    ) and os.path.isfile(
        os.path.join(bot.config.homedir, 'GeoIPASNum.dat')
    ):
        return bot.config.homedir
    elif os.path.isfile(
        os.path.join('/usr/share/GeoIP', 'GeoLiteCity.dat')
    ) and os.path.isfile(
        os.path.join('/usr/share/GeoIP', 'GeoIPASNum.dat')
    ):
...
def ip(bot, trigger):
    ...
    if db_path is False:
        bot.debug(
            __file__,
            "Can't find (or download) usable GeoIP database",
            'always'
        )
