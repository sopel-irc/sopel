...
Note that this will no longer work with the Google Translation API,
but could be refactored to work with another translation API, like the
one provided by Bing.
...
def translate(text, in_lang='auto', out_lang='en'):
    ...
    query_string = "&".join(
        "{key}={value}".format(key=key, value=web.quote(value))
