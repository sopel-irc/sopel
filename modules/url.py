...
def show_title_auto (jenni, input):
    if (input.startswith('.topic ') or input.startswith('.tmask ') or input.startswith('.title ') or input.startswith('.bitly ') or input.startswith('.dftba') or re.match('.*(youtube.com/watch\S*v=|youtu.be/)([\w-]+.*)', input)) or re.match('.*(http(?:s)?://(www\.)?reddit\.com/r/.*?/comments/[\w-]+).*', input):
        return
